...
const faq = [
  "Можно ли перейти с «Старт» на «Рост» без потери данных?",
      ...
      <Section eyebrow="Тарифы" title="Планы для старта и масштабирования" text="Без скрытых платежей, с понятными лимитами и быстрым стартом.">
        <div className="mb-6 flex items-center justify-between gap-3 rounded-2xl border border-line bg-white p-3">
          <div className="inline-flex rounded-2xl border border-line bg-paper p-1">
            <button onClick={() => setBilling("monthly")} className={`h-9 rounded-xl px-3 text-sm font-semibold ${billing === "monthly" ? "bg-ink text-white" : ""}`}>Помесячно</button>
            <button onClick={() => setBilling("yearly")} className={`h-9 rounded-xl px-3 text-sm font-semibold ${billing === "yearly" ? "bg-ink text-white" : ""}`}>За год -20%</button>
          </div>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <PricingCard name="Старт" price={yearly ? "0 ₽" : "0 ₽"} text="Тест ниши, базовая витрина и мастер запуска." />
          <PricingCard name="Рост" price={yearly ? "2 320 ₽" : "2 900 ₽"} text="Лучший для растущего бизнеса: расширенный кабинет, AI-улучшения и больше шаблонов." featured />
          <PricingCard name="Бизнес" price={yearly ? "6 320 ₽" : "7 900 ₽"} text="Командная работа, несколько витрин и приоритетная поддержка." />
        </div>
